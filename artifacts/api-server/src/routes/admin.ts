...
function parseFile(buffer: Buffer, originalName: string): { headers: string[]; rows: Record<string, string>[] } {
  ...
  if (ext.endsWith(".csv")) {
    ...
    return { headers: Object.keys(data[0]), rows: data as Record<string, string>[] };
  }
...
const TARGET_FIELDS = [
  "facility_name","contact_name","phone","email","address","city","state","level_of_care","notes",
] as const;
    ...
    for (const row of rows) {
      try {
        ...
        const [existing] = await db
          ...
          .where(and(eq(referralSources.name, name), eq(referralSources.companyId, companyId)))
          .limit(1);
