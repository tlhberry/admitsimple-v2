...
import { Loader2, TrendingUp, BarChart2, Users, DollarSign, Building2, Activity, CalendarDays, ChevronDown } from "lucide-react";
import {
...
function formatLabel(startDate: string, endDate: string, preset: Preset) {
  if (preset === "Custom") {
    const fmt = (s: string) => {
...
export default function Analytics() {
  ...
  const [customStart, setCustomStart] = useState(() => toISO(new Date(Date.now() - 30 * 86400000)));
  ...
  const { startDate, endDate } = useMemo(() => {
    ...
    return {
      startDate: toISO(new Date(Date.now() - p.days! * 86400000)),
      endDate:   toISO(new Date()),
    };
  }, [preset, customStart, customEnd]);

  const { data, isLoading } = useGetChartData({ startDate, endDate });

  const { data: bdAnalytics, isLoading: bdLoading } = useQuery<any>({
    queryKey: ["/api/bd-analytics", startDate, endDate],
    queryFn: () =>
      fetch(`/api/bd-analytics?startDate=${startDate}&endDate=${endDate}`, { credentials: "include" }).then(r => r.json()),
    staleTime: 60000,
  });

  const periodLabel = formatLabel(startDate, endDate, preset);
            ...
            {preset === "Custom" && (
              <div className="flex items-center gap-2 text-xs">
                <input
                  ...
                  className="bg-muted border border-border rounded-lg px-2 py-1.5 text-foreground text-xs focus:outline-none focus:ring-1 focus:ring-primary"
                />
          ...
          <div className="mt-8">
            <div className="mb-4">
              ...
              <p className="text-muted-foreground text-sm mt-0.5">
                Referral account and BD activity metrics for {periodLabel}.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
              <Card className="rounded-2xl border-border bg-card">
                <CardContent className="p-5">
                  ...
                  <p className="text-3xl font-bold text-foreground">{bdLoading ? "—" : (bdAnalytics?.totalAccounts ?? "—")}</p>
                </CardContent>
              </Card>
              <Card className="rounded-2xl border-border bg-card">
                <CardContent className="p-5">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-muted-foreground font-medium">Active Accounts (period)</span>
                    <Activity className="w-4 h-4 text-emerald-400" />
                  </div>
                  <p className="text-3xl font-bold text-foreground">{bdLoading ? "—" : (bdAnalytics?.activeAccounts30 ?? "—")}</p>
                </CardContent>
              </Card>
              <Card className="rounded-2xl border-border bg-card">
                <CardContent className="p-5">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-muted-foreground font-medium">Total Activities (period)</span>
                    <BarChart2 className="w-4 h-4 text-amber-400" />
                  </div>
                  <p className="text-3xl font-bold text-foreground">{bdLoading ? "—" : (bdAnalytics?.totalActivities30 ?? "—")}</p>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
              ...
              <Card className="rounded-2xl border-border overflow-hidden">
                <CardHeader className="bg-muted/40 border-b border-border pb-3">
                  <CardTitle className="text-sm font-semibold flex items-center gap-2 text-foreground">
                    <Activity className="w-4 h-4 text-primary" /> Activities by Type
                  </CardTitle>
                </CardHeader>
            ...
            {/* Top BD Reps */}
            {!bdLoading && (bdAnalytics?.topBdReps?.length > 0) && (
              <Card className="rounded-2xl border-border overflow-hidden">
                <CardHeader className="bg-muted/40 border-b border-border pb-3">
                  <CardTitle className="text-sm font-semibold flex items-center gap-2 text-foreground">
                    <Users className="w-4 h-4 text-emerald-400" /> Top BD Reps
                  </CardTitle>
                </CardHeader>
